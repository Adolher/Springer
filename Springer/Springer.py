...
l = Label(root, height=5, width=80)
l.grid(row=8, column=0, columnspan=8)
l["text"] = "Lösungen gesamt: 0"

schieber=Label(root, height=45, width=30)
schieber.grid(row=0, column=8, rowspan=9)

def sch_anzeigen(self):
...
def extstart(x,y):
    t=threading.Thread(target=funkt.move, args =(brett, zug, x, y, erg_list, l, scvwert))
    t.start()
